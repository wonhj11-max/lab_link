...
begin
  ...
  for entry in select value from jsonb_array_elements(p_entries) loop
    ...
    if source_host is distinct from homepage_host and not (source_host like '%.unist.ac.kr' and homepage_host like '%.unist.ac.kr') then
      raise exception 'Official source host does not match lab homepage';
    end if;
    ...
    select j.id into v_journal_id from public.journals j
    ...
    values(lower(entry->>'doi'),entry->>'title',(entry->>'publication_year')::integer,entry->>'journal',
      'https://doi.org/'||lower(entry->>'doi'),v_journal_id,
    ...
    values(lab.id,v_paper_id,lab.professor,case when faculty_corresponding then 'confirmed' else 'unknown' end,
    ...
    for author in select value from jsonb_array_elements(entry->'authors') loop
      ...
      values(v_paper_id,(author->>'order')::integer,author->>'name',nullif(author#>>'{affiliations,0}',''),
    ...
    values(lab.id,(entry->>'listing_year')::integer,lower(entry->>'doi'),entry->>'title',entry->>'journal_citation',
      entry->'authors',entry->>'source_url','https://doi.org/'||lower(entry->>'doi'),entry,now(),v_paper_id,'published',
    ...
    insert into private.audit_logs(action,entity_id,detail) values('verified_bibliography_published',v_paper_id,
